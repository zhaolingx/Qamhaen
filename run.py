...
class Experiment:
    ...
    def predict_leave1out_10group(self, dataset_json, embedding_method, times, algorithm, comparison, para=None, Sanity=False, QualityOfMessage=False, RQ=None):
        ...
        if RQ == 'RQ1' and not Sanity and not QualityOfMessage:
            print('############################################')
            if para == '':
                print('RQ1, Effectiveness of Qamhaen.')
                ...
                print('RQ1-balance, Effectiveness of Qamhaen.')
            ...
            if para == '':
                print('#####################')
                print('Table 2: Confusion matrix of Qamhaen prediction.')
            _, _, _, _, _, f1_qamhaen, auc_qamhaen = self.evaluation_metrics(NLP_model_ytests, NLP_model_preds, RQ)
            print('---------------')
            ...
            recall_p_qamhaen, recall_n_qamhaen = new_matrix_average[4][3], new_matrix_average[5][3]
        ...
        if RQ == 'RQ2.2' and Sanity:
            ...
            print('All correct: {}, Correct by Qamhaen: {}, Correct by Random:{}'.format(all_correct, cnt_all_predict_correct, cnt_all_random_correct))
            ...
            self.boxplot_distribution_correlation(distribution_prob, 'Prediction probability by Qamhaen', 'Original pairs', 'Random pairs', 'figure8_bug_report_quality.jpg')
        ...
        if RQ == 'RQ2.3':
            ...
            if para == '' and QualityOfMessage:
                ...
                _, _, _, _, _, f1_qamhaen, auc_qamhaen = self.evaluation_metrics(NLP_model_ytests, NLP_model_preds)
                print('---------------')

        if RQ == 'RQ3':
            _, _, _, _, _, f1_qamhaen, auc_qamhaen = self.evaluation_metrics_noprint(NLP_model_ytests, NLP_model_preds)
            ...
            if comparison == 'DL':
                ...
                if para == 'lr':
                    ...
                    qua_re = 'Qamhaen:                {}:{} | {:.3f} {:.3f}  {:.3f}   {:.3f}'.format(incorrect_number, correct_number, auc_qamhaen, f1_qamhaen,  recall_p_qamhaen, recall_n_qamhaen)
                    return lr_re, qua_re
                elif para == 'rf':
                    rf_re = 'Tian et al. (RF):        {}:{} | {:.3f} {:.3f}  {:.3f}   {:.3f}'.format(incorrect_number, correct_number, auc_, f1, recall_positive, recall_negative,)
                    exclusively_re = '{} out of {} patches are exclusively identified by Qamhaen.'.format(new_identify_cnt, identify_cnt,)
        ...
        for test_id in test_ids:
            ...
            for v in range(1, len(value)):
                ...
                if Sanity:
                    ...
                    # count test data of compared approaches earlier than Qamhaen, keep the test dataset size same.
                    if comparison == 'DL':
                        for p in range(len(ASE_value)):
                            patch_id, ASE_vector, ASE_label = ASE_value[p][0], ASE_value[p][1], ASE_value[p][2]
...
if __name__ == '__main__':
    ...
    if arg1 == 'hypothesis':
        ...
        result_lr, result_qamhaen = e.predict_leave1out_10group(dataset_json, embedding, times=10, algorithm='qa_attetion', comparison='DL', para='lr', Sanity=False, QualityOfMessage=False, RQ=arg1)
        ...
        print('############################################')
        print('RQ3-DL, Table 3: Qamhaen vs a DL-based patch classifier.')
        print('——————————————————————————————————————————————————————————————————————')
        ...
        print('############################################')
        print('RQ3-BATS, Table 4: Qamhaen vs BATS.')
        print('——————————————————————————————————————————————————————————————————————')
        print('Classifier       Incorrect:Correct |   AUC    F1 +Recall -Recall')
        print('——————————————————————————————————————————————————————————————————————')
        print(result_00)
        ...
        result_patchsim, result_qamhaen, exclusively_re = e.predict_leave1out_10group(dataset_json, embedding, times=10, algorithm='qa_attetion', comparison='PATCHSIM', para='v1', Sanity=False, QualityOfMessage=False, RQ=arg1)
        print('############################################')
        print('RQ3-PATCHSIM, Table 5: Qamhaen vs (execution-based) PATCH-SIM.')
        print('——————————————————————————————————————————————————————————————————————')
        print('Classifier       Incorrect:Correct |   AUC    F1 +Recall -Recall')
        print('——————————————————————————————————————————————————————————————————————')
